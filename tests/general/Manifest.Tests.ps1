Describe "Validating the module manifest" {
	$moduleRoot = (Resolve-Path "$global:testroot\..\PowerShellForGitHub.Content").Path
	$manifest = ((Get-Content "$moduleRoot\PowerShellForGitHub.Content.psd1") -join "`n") | Invoke-Expression
